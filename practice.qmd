...
map_dfr(outcome_list, function(x) {
  tibble(
    ...
    unit_of_measure = pluck(x, "unitOfMeasure"),
    time_frame = pluck(x, "timeFrame"),
    group_id = paste(c(x$groups[[1]]$id, x$groups[[2]]$id), collapse = ";"),
    group_title = paste(c(x$groups[[1]]$title, x$groups[[2]]$title), collapse = ";"),
    group_description = paste(c(x$groups[[1]]$description, x$groups[[2]]$description), collapse = ";"),
  ) %>% 
    unlist()
})
```
...
testing %>% 
  separate_longer_delim(
    cols = c(group_id, group_title, group_description),
    delim = ";"
  )
...
test %>% 
  # filter(event != "Number of attacks treated" & event != "VAS scale" & event != "Patients Who Used Any Type of Rescue Medication" & event != "Disability score")  %>% 
  filter(title == "Mean Change of Questionnaire EQ-5D-3L (Euroqol- 5D-3L) From Baseline to After 2 Weeks Treatment") %>% 
  ggplot(aes(x = time, y = value, group = group_title, color = group_title)) +
  scale_color_brewer("Group", palette = "Set1") +
  geom_line(linewidth = 1, alpha = 0.7) +
  geom_point(size = 3, alpha = 0.7) +
...
saveRDS(test, "screencast-app/data/first_study_outcome_measures.rds")
